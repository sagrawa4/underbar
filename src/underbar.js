(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    var iteratedValues = [];
    var noDuplicateArrayIterators = [];

    if (typeof iterator !== 'function') {
      _.each(array, function(item) {
        if (hashObject[item] === undefined) {
          hashObject[item] = 1;
      ...
      _.each(array, function(item) {
        iteratedValues.push(iterator(item));
      });

      for (var i in array) {
        hashObject[array[i]] = iteratedValues[i];
      }

      _.each(hashObject, function(item, i) {
        if (_.indexOf(noDuplicateArrayIterators, hashObject[i]) === -1) {
          noDuplicateArrayIterators.push(hashObject[i]);
          noDuplicateArray.push(Number(i));
        }
      });
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(match, item) {
      if (match) {
        var postIteratorValue = iterator(item);
        if (!postIteratorValue) {
          return false;
        } else {
          return true;
        }
  ...
  _.some = function(collection, iterator) {
    ...
    var result = _.reduce(collection, function(match, item) {
      if (!match) {
        if (iterator(item)) {
          return true;
        } else {
  ...
  _.delay = function(func, wait) {
    ...
    if (arguments.length > 2) {
      ...
      return setTimeout(func.apply(null, arrayOfArguments), wait);
    } else {
